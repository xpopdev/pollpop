...
- **Security review — Critical/High mitigated, Medium/Low remain (honest):** red-team 2026-08-14 filed 16 SEC + 24 BUG (`company/engineering/security/red-team-2026-08-14.md`, `company/engineering/tests/red-team-2026-08-14.md`). FIXED: 003 RLS tighten (anon SELECT only, service_role writes via `supaService()`), 004 storage bucket `poll-images` public + policies applied — data URLs now uploaded to Storage, not stored as unbounded `text` (removed from limitations), 002 vote RPC atomic `increment_vote`/`decrement_vote` + orphan poll cleanup on option insert fail, create 5/hr + vote 10/poll/24h rate limits (mock + Supabase), meta cap 2KB (`capMeta` in `store.ts` + `route.ts`), atomic persist via tmp→rename. Remaining Medium/Low back into Phase B: PollCard gradient P1 — overlay `linear-gradient(to top, rgba(0,0,0,.55) 0%, transparent 55%)` functional, P1 contrast tuning for very light images deferred; Realtime publication verify live (Medium RT-BUG-16: requires `alter publication supabase_realtime add table poll_options;` in dashboard, PollClient uses `supa.removeChannel` + 5s polling fallback until live probe confirms); optimistic reconcile (Medium RT-BUG-15: PollClient toasts only after 200 + reverts on error, but cross-tab/50-concurrent can show transient stale count until Realtime/5s poll reconciles); cookie trust (Medium RT-SEC-05/RT-BUG-17: `pollpop_cid` is JS-readable `SameSite=Lax` no HttpOnly, `x-pollpop-cid` header accepted as fallback, `x-forwarded-for` first-entry trusted — harden to HttpOnly `Set-Cookie` + `x-vercel-forwarded-for`/`req.ip` next).
...
- **Failure modes not fully tested:** Realtime down (fallback to 5s poll), image upload fail, NSFW provider down, 429 Retry-After — file-reviewed but not chaos-tested live.
- **Docs not confirmed:** `technical-writer` not consulted.
- **Next:** Fix install env (move `app/` outside `/storage/emulated/0` or standard Linux/Node 18-20), `qa-lead` live runs + Realtime/poll_view dedup verification, harden cookie trust + profanity/NSFW P1, `validate-hypothesis` on live CTR/K, then re-run `quality-bar`. Once every §33 item genuinely checked, milestone in `history/milestones/` and §40 continuous improvement.
