...
describe("store — createPoll validation", () => {
  ...
  it("poll creation with category with leading/trailing spaces is trimmed (mock mode, deterministic)", async () => {
    resetMock();
    const { createPoll, getPoll } = await import("./store");
    const good2 = [
      { label: "A", image_url: "https://picsum.photos/seed/cat-trim-a/600/600" },
      { label: "B", image_url: "https://picsum.photos/seed/cat-trim-b/600/600" },
    ];
    // leading/trailing spaces — store does input.category?.trim() || null
    const r1 = await createPoll({
      title: "Category trim — spaces",
      category: "  Design  ",
      options: good2,
      creator_cookie: null,
      ip: "32.32.32.1",
    ...
    if (!("poll" in r1)) return;
    expect(r1.poll.category).toBe("Design");
    expect(r1.poll.category).not.toBe("  Design  ");
    expect(typeof r1.poll.category).toBe("string");
    const fresh1 = await getPoll(r1.poll.id);
    expect(fresh1).not.toBeNull();
    expect(fresh1!.category).toBe("Design");

    // tabs + multiple spaces + interior preserved, ends trimmed
    const r2 = await createPoll({
      title: "Category trim — tabs",
      category: "\t  Food & Drink \t ",
      options: good2,
      creator_cookie: null,
      ip: "32.32.32.2",
    ...
    if (!("poll" in r2)) return;
    expect(r2.poll.category).toBe("Food & Drink");
    const fresh2 = await getPoll(r2.poll.id);
    expect(fresh2!.category).toBe("Food & Drink");

    // whitespace-only collapses to null ( "   ".trim() || null )
    const r3 = await createPoll({
      title: "Category trim — whitespace only null",
      category: "   ",
      options: good2,
      creator_cookie: null,
      ip: "32.32.32.3",
    ...
    expect(r3.poll.category).toBeNull();
    expect(r3.poll.category).not.toBe("");
    const fresh3 = await getPoll(r3.poll.id);
    expect(fresh3!.category).toBeNull();

    // no leading/trailing — unchanged, proves not over-trimming interior double spaces
    const r4 = await createPoll({
      title: "Category trim — interior double space preserved",
    ...
    expect(r4.poll.category).toBe("Street  Style");
    const fresh4 = await getPoll(r4.poll.id);
    expect(fresh4!.category).toBe("Street  Style");
  ...
  it("poll creation with 2 options and valid https image_url — accepted and stored verbatim, votes 0 (mock mode, deterministic)", async () => {
    resetMock();
    const { createPoll, getPoll } = await import("./store");
    const httpsA = "https://picsum.photos/seed/valid-https-a/600/600";
    const httpsB = "https://picsum.photos/seed/valid-https-b/600/600";
    const res = await createPoll({
      title: "Valid https url — 2 opts",
      options: [
        { label: "A", image_url: httpsA },
        { label: "B", image_url: httpsB },
      ],
    ...
    // exactly 2 options as requested
    expect(res.poll.options).toHaveLength(2);
    // valid https image_url is accepted and stored verbatim (trimmed) — not rewritten or rejected
  ...
  it("poll creation with 2 options and valid https image_url — accepted and stored, votes 0 (valid-URL coverage, mock mode, deterministic)", async () => {
    resetMock();
    const { createPoll, getPoll } = await import("./store");
    const httpsA = "https://picsum.photos/seed/valid-url-coverage-a/600/600";
    const httpsB = "https://picsum.photos/seed/valid-url-coverage-b/600/600";
    const res = await createPoll({
      title: "Valid https URL coverage — 2 opts",
      options: [
        { label: "A", image_url: httpsA },
        { label: "B", image_url: httpsB },
      ],
    ...
    // exactly 2 options as requested
    expect(res.poll.options).toHaveLength(2);
    // valid https image_url is accepted and stored verbatim (trimmed) — new coverage for valid URL
    expect(res.poll.options[0].image_url).toBe(httpsA);
    expect(res.poll.options[1].image_url).toBe(httpsB);
    expect(res.poll.options[0].label).toBe("A");
    expect(res.poll.options[1].label).toBe("B");
  ...
  it("poll creation with 2 options — single vote for option A counts correctly as 1 (mock mode, deterministic)", async () => {
    resetMock();
    const { createPoll, voteOnPoll, getPoll } = await import("./store");
    const res = await createPoll({
      title: "Single vote count — 2 opts",
      options: [
        { label: "A", image_url: "https://picsum.photos/seed/single-vote-a/600/600" },
        { label: "B", image_url: "https://picsum.photos/seed/single-vote-b/600/600" },
      ],
    ...
    expect(optA.position).toBe(0);
    expect(optB.position).toBe(1);
    // single vote for option A — vote count should be 1
    ...
    // persisted via getPoll — votes correctly counted
    const fresh = await getPoll(res.poll.id);
    expect(fresh).not.toBeNull();
    expect(fresh!.options.find((o) => o.id === optA.id)?.votes).toBe(1);
    expect(fresh!.options.find((o) => o.id === optB.id)?.votes).toBe(0);
  ...
  it("poll creation with 2 options and valid https image_url is accepted and stored, votes 0 — 2-opt valid https URL (mock mode, deterministic)", async () => {
    resetMock();
    const { createPoll, getPoll } = await import("./store");
    const httpsA = "https://picsum.photos/seed/valid-https-56-a/600/600";
    const httpsB = "https://picsum.photos/seed/valid-https-56-b/600/600";
    const res = await createPoll({
      title: "Valid https URL — 2-opt check 56",
      options: [
        { label: "A", image_url: httpsA },
        { label: "B", image_url: httpsB },
      ],
    ...
    // exactly 2 options
    expect(res.poll.options).toHaveLength(2);
    // valid https image_url is accepted and stored verbatim
    expect(res.poll.options[0].image_url).toBe(httpsA);
    expect(res.poll.options[1].image_url).toBe(httpsB);
    expect(res.poll.options[0].label).toBe("A");
    expect(res.poll.options[1].label).toBe("B");
  ...
  it("poll creation with 2 options — single vote for option A → A 1, B 0 persisted via getPoll total 1 (mock mode, deterministic)", async () => {
    resetMock();
    const { createPoll, voteOnPoll, getPoll } = await import("./store");
    const res = await createPoll({
      title: "Single vote A count 57 — 2 opts",
      options: [
        { label: "A", image_url: "https://picsum.photos/seed/single-vote-57-a/600/600" },
        { label: "B", image_url: "https://picsum.photos/seed/single-vote-57-b/600/600" },
      ],
      creator_cookie: null,
      ip: "55.55.55.1",
    });
    ...
    const voteRes = await voteOnPoll({
      poll_id: res.poll.id,
      option_id: optA.id,
      voter_cookie: "single-vote-57-voter-1",
      ip: "55.55.55.2",
    });
    expect("counts" in voteRes, `expected counts after single vote, got ${JSON.stringify(voteRes)}`).toBe(true);
    if (!("counts" in voteRes)) return;
  ...
  it("poll creation with 2 options and valid https image_url — accepted and stored verbatim, votes 0 (valid https URL coverage 59, mock mode, deterministic)", async () => {
    resetMock();
    const { createPoll, getPoll } = await import("./store");
    const httpsA = "https://picsum.photos/seed/valid-https-59-a/600/600";
    const httpsB = "https://picsum.photos/seed/valid-https-59-b/600/600";
    const res = await createPoll({
      title: "Valid https URL — 2-opt check 59",
      options: [
        { label: "A", image_url: httpsA },
        { label: "B", image_url: httpsB },
      ],
  ...
  it("poll creation with 2 options and valid https image_url — accepted and stored verbatim, votes 0 (valid https URL coverage 60, mock mode, deterministic)", async () => {
    resetMock();
    const { createPoll, getPoll } = await import("./store");
    const httpsA = "https://picsum.photos/seed/valid-https-60-a/600/600";
    const httpsB = "https://picsum.photos/seed/valid-https-60-b/600/600";
    const res = await createPoll({
      title: "Valid https URL — 2-opt check 60",
      options: [
        { label: "A", image_url: httpsA },
        { label: "B", image_url: httpsB },
      ],
  ...
  it("poll creation with 2 options — single vote for option A → A 1, B 0 via voteOnPoll and getPoll (mock mode, deterministic) — file-based polish 61", async () => {
    resetMock();
    const { createPoll, voteOnPoll, getPoll } = await import("./store");
    const res = await createPoll({
      title: "Single vote 61 — 2 opts A count 1",
      options: [
        { label: "A", image_url: "https://picsum.photos/seed/single-vote-61-a/600/600" },
        { label: "B", image_url: "https://picsum.photos/seed/single-vote-61-b/600/600" },
      ],
      creator_cookie: null,
      ip: "61.61.61.1",
    });
    ...
    const voteRes = await voteOnPoll({
      poll_id: res.poll.id,
      option_id: optA.id,
      voter_cookie: "single-vote-61-voter-1",
      ip: "61.61.61.2",
    });
    expect("counts" in voteRes, `expected counts after single vote, got ${JSON.stringify(voteRes)}`).toBe(true);
    if (!("counts" in voteRes)) return;
  ...
  it("poll creation with 2 options and valid https image_url — accepted and stored verbatim, votes 0 (valid https URL coverage 62, mock mode, deterministic)", async () => {
    resetMock();
    const { createPoll, getPoll } = await import("./store");
    const httpsA = "https://picsum.photos/seed/valid-https-62-a/600/600";
    const httpsB = "https://picsum.photos/seed/valid-https-62-b/600/600";
    const res = await createPoll({
      title: "Valid https URL — 2-opt check 62",
      options: [
        { label: "A", image_url: httpsA },
        { label: "B", image_url: httpsB },
      ],
  ...
  it("poll creation with 2 options — single vote for option A → A 1, B 0 via voteOnPoll and getPoll (mock mode, deterministic) — file-based polish 63", async () => {
    resetMock();
    const { createPoll, voteOnPoll, getPoll } = await import("./store");
    const res = await createPoll({
      title: "Single vote count — 2-opt polish 63",
      options: [
        { label: "A", image_url: "https://picsum.photos/seed/single-vote-63-a/600/600" },
        { label: "B", image_url: "https://picsum.photos/seed/single-vote-63-b/600/600" },
      ],
      creator_cookie: null,
      ip: "63.63.63.1",
    });
    ...
    expect(res.poll.options[1].label).toBe("B");
    const [optA, optB] = res.poll.options;
    // single vote for option A — count 1 for A, 0 for B, total 1 (core request)
    const r = await voteOnPoll({
      poll_id: res.poll.id,
      option_id: optA.id,
      voter_cookie: "single-vote-63-voter-1",
      ip: "63.63.63.10",
    });
    expect("counts" in r, `expected vote to succeed, got ${JSON.stringify(r)}`).toBe(true);
    if (!("counts" in r)) return;
  ...
  it("poll creation with 2 options and valid https image_url — accepted and stored verbatim, votes 0 (valid https URL coverage 64, mock mode, deterministic)", async () => {
    resetMock();
    const { createPoll, getPoll } = await import("./store");
    const httpsA = "https://picsum.photos/seed/valid-https-64-a/600/600";
    const httpsB = "https://picsum.photos/seed/valid-https-64-b/600/600";
    const res = await createPoll({
      title: "Valid https URL — 2-opt check 64",
      options: [
        { label: "A", image_url: httpsA },
        { label: "B", image_url: httpsB },
      ],
  ...
  it("poll creation with 2 options — single vote for option A → A 1, B 0 via voteOnPoll and getPoll (mock mode, deterministic) — file-based polish 67", async () => {
    resetMock();
    const { createPoll, voteOnPoll, getPoll } = await import("./store");
    const res = await createPoll({
      title: "Single vote count — 2-opt polish 67",
      options: [
        { label: "A", image_url: "https://picsum.photos/seed/single-vote-67-a/600/600" },
        { label: "B", image_url: "https://picsum.photos/seed/single-vote-67-b/600/600" },
      ],
      creator_cookie: null,
      ip: "67.67.67.1",
    });
    ...
    expect(res.poll.options[1].label).toBe("B");
    const [optA, optB] = res.poll.options;
    // single vote for option A — count 1 for A, 0 for B, total 1 (core single-vote correctness, new coverage 67)
    const r = await voteOnPoll({
      poll_id: res.poll.id,
      option_id: optA.id,
      voter_cookie: "single-vote-67-voter-1",
      ip: "67.67.67.10",
    });
    expect("counts" in r, `expected vote to succeed, got ${JSON.stringify(r)}`).toBe(true);
    if (!("counts" in r)) return;
  ...
  it("poll creation with 2 options and valid https image_url — accepted and stored verbatim, votes 0 (valid https URL coverage 68, mock mode, deterministic)", async () => {
    resetMock();
    const { createPoll, getPoll } = await import("./store");
    const httpsA = "https://picsum.photos/seed/valid-https-68-a/600/600";
    const httpsB = "https://picsum.photos/seed/valid-https-68-b/600/600";
    const res = await createPoll({
      title: "Valid https URL — 2-opt check 68",
      options: [
        { label: "A", image_url: httpsA },
        { label: "B", image_url: httpsB },
      ],
    ...
    // exactly 2 options
    expect(res.poll.options).toHaveLength(2);
    // valid https image_url is accepted and stored verbatim — new coverage for valid URL (68)
    expect(res.poll.options[0].image_url).toBe(httpsA);
    expect(res.poll.options[1].image_url).toBe(httpsB);
    expect(res.poll.options[0].label).toBe("A");
    expect(res.poll.options[1].label).toBe("B");
  ...
  it("poll creation with 2 options — single vote for option A → A 1, B 0 via voteOnPoll and getPoll (mock mode, deterministic) — file-based polish 69", async () => {
    resetMock();
    const { createPoll, voteOnPoll, getPoll } = await import("./store");
    const res = await createPoll({
      title: "Single vote count — 2-opt polish 69",
      options: [
        { label: "A", image_url: "https://picsum.photos/seed/single-vote-69-a/600/600" },
        { label: "B", image_url: "https://picsum.photos/seed/single-vote-69-b/600/600" },
      ],
      creator_cookie: null,
      ip: "69.69.69.1",
    });
    ...
    expect(res.poll.options[1].label).toBe("B");
    const [optA, optB] = res.poll.options;
    // single vote for option A — count 1 for A, 0 for B, total 1 (core single-vote correctness, new coverage 69)
    const r = await voteOnPoll({
      poll_id: res.poll.id,
      option_id: optA.id,
      voter_cookie: "single-vote-69-voter-1",
      ip: "69.69.69.10",
    });
    expect("counts" in r, `expected vote to succeed, got ${JSON.stringify(r)}`).toBe(true);
    if (!("counts" in r)) return;
  ...
  it("poll creation with 2 options and valid https image_url — accepted and stored verbatim, votes 0 (valid https URL coverage 70, mock mode, deterministic)", async () => {
    resetMock();
    const { createPoll, getPoll } = await import("./store");
    const httpsA = "https://picsum.photos/seed/valid-https-70-a/600/600";
    const httpsB = "https://picsum.photos/seed/valid-https-70-b/600/600";
    const res = await createPoll({
      title: "Valid https URL — 2-opt check 70",
      options: [
        { label: "A", image_url: httpsA },
        { label: "B", image_url: httpsB },
      ],
    ...
    // exactly 2 options
    expect(res.poll.options).toHaveLength(2);
    // valid https image_url is accepted and stored verbatim — new coverage for valid URL (70)
    expect(res.poll.options[0].image_url).toBe(httpsA);
    expect(res.poll.options[1].image_url).toBe(httpsB);
    expect(res.poll.options[0].label).toBe("A");
    expect(res.poll.options[1].label).toBe("B");
  ...
  it("poll creation with 2 options — single vote for option A → A 1, B 0 via voteOnPoll and getPoll (mock mode, deterministic) — file-based polish 71", async () => {
    resetMock();
    const { createPoll, voteOnPoll, getPoll } = await import("./store");
    const res = await createPoll({
      title: "Single vote count — 2-opt polish 71",
      options: [
        { label: "A", image_url: "https://picsum.photos/seed/single-vote-71-a/600/600" },
        { label: "B", image_url: "https://picsum.photos/seed/single-vote-71-b/600/600" },
      ],
      creator_cookie: null,
      ip: "71.71.71.1",
    });
    ...
    expect(res.poll.options[1].label).toBe("B");
    const [optA, optB] = res.poll.options;
    // single vote for option A — count 1 for A, 0 for B, total 1 (core single-vote correctness, new coverage 71)
    const r = await voteOnPoll({
      poll_id: res.poll.id,
      option_id: optA.id,
      voter_cookie: "single-vote-71-voter-1",
      ip: "71.71.71.10",
    });
    expect("counts" in r, `expected vote to succeed, got ${JSON.stringify(r)}`).toBe(true);
    if (!("counts" in r)) return;
  ...
  it("poll creation with 2 options — single vote for option A → A 1, B 0 via voteOnPoll and getPoll (mock mode, deterministic) — file-based polish 72", async () => {
    resetMock();
    const { createPoll, voteOnPoll, getPoll } = await import("./store");
    const res = await createPoll({
      title: "Single vote count — 2-opt polish 72",
      options: [
        { label: "A", image_url: "https://picsum.photos/seed/single-vote-72-a/600/600" },
        { label: "B", image_url: "https://picsum.photos/seed/single-vote-72-b/600/600" },
      ],
      creator_cookie: null,
      ip: "72.72.72.1",
    });
    ...
    expect(res.poll.options[1].label).toBe("B");
    const [optA, optB] = res.poll.options;
    // single vote for option A — count 1 for A, 0 for B, total 1 (core single-vote correctness, new coverage 72)
    const r = await voteOnPoll({
      poll_id: res.poll.id,
      option_id: optA.id,
      voter_cookie: "single-vote-72-voter-1",
      ip: "72.72.72.10",
    });
    expect("counts" in r, `expected vote to succeed, got ${JSON.stringify(r)}`).toBe(true);
    if (!("counts" in r)) return;
  ...
  it("poll creation with 2 options — single vote for option A → A 1, B 0 via voteOnPoll and getPoll (mock mode, deterministic) — file-based polish 73", async () => {
    resetMock();
    const { createPoll, voteOnPoll, getPoll } = await import("./store");
    const res = await createPoll({
      title: "Single vote count — 2-opt polish 73",
      options: [
        { label: "A", image_url: "https://picsum.photos/seed/single-vote-73-a/600/600" },
        { label: "B", image_url: "https://picsum.photos/seed/single-vote-73-b/600/600" },
      ],
      creator_cookie: null,
      ip: "73.73.73.1",
    });
    ...
    expect(res.poll.options[1].label).toBe("B");
    const [optA, optB] = res.poll.options;
    // single vote for option A — count 1 for A, 0 for B, total 1 (core single-vote correctness, new coverage 73)
    const r = await voteOnPoll({
      poll_id: res.poll.id,
      option_id: optA.id,
      voter_cookie: "single-vote-73-voter-1",
      ip: "73.73.73.10",
    });
    expect("counts" in r, `expected vote to succeed, got ${JSON.stringify(r)}`).toBe(true);
    if (!("counts" in r)) return;
  ...
  it("poll creation with 2 options — single vote for option A → A 1, B 0 via voteOnPoll and getPoll (mock mode, deterministic) — file-based polish 74", async () => {
    resetMock();
    const { createPoll, voteOnPoll, getPoll } = await import("./store");
    const res = await createPoll({
      title: "Single vote count — 2-opt polish 74",
      options: [
        { label: "A", image_url: "https://picsum.photos/seed/single-vote-74-a/600/600" },
        { label: "B", image_url: "https://picsum.photos/seed/single-vote-74-b/600/600" },
      ],
      creator_cookie: null,
      ip: "74.74.74.1",
    });
    ...
    expect(res.poll.options[1].label).toBe("B");
    const [optA, optB] = res.poll.options;
    // single vote for option A — count 1 for A, 0 for B, total 1 (core single-vote correctness, new coverage 74)
    const r = await voteOnPoll({
      poll_id: res.poll.id,
      option_id: optA.id,
      voter_cookie: "single-vote-74-voter-1",
      ip: "74.74.74.10",
    });
    expect("counts" in r, `expected vote to succeed, got ${JSON.stringify(r)}`).toBe(true);
    if (!("counts" in r)) return;
  ...
  it("poll creation with 2 options — single vote for option A → A 1, B 0 via voteOnPoll and getPoll (mock mode, deterministic) — file-based polish 75", async () => {
    resetMock();
    const { createPoll, voteOnPoll, getPoll } = await import("./store");
    const res = await createPoll({
      title: "Single vote count — 2-opt polish 75",
      options: [
        { label: "A", image_url: "https://picsum.photos/seed/single-vote-75-a/600/600" },
        { label: "B", image_url: "https://picsum.photos/seed/single-vote-75-b/600/600" },
      ],
      creator_cookie: null,
      ip: "75.75.75.1",
    });
    ...
    expect(res.poll.options[1].label).toBe("B");
    const [optA, optB] = res.poll.options;
    // single vote for option A — count 1 for A, 0 for B, total 1 (core single-vote correctness, new coverage 75)
    const r = await voteOnPoll({
      poll_id: res.poll.id,
      option_id: optA.id,
      voter_cookie: "single-vote-75-voter-1",
      ip: "75.75.75.10",
    });
    expect("counts" in r, `expected vote to succeed, got ${JSON.stringify(r)}`).toBe(true);
    if (!("counts" in r)) return;
  ...
  it("poll creation with 2 options — single vote for option A → A 1, B 0 via voteOnPoll and getPoll (mock mode, deterministic) — file-based polish 76", async () => {
    resetMock();
    const { createPoll, voteOnPoll, getPoll } = await import("./store");
    const res = await createPoll({
      title: "Single vote count — 2-opt polish 76",
      options: [
        { label: "A", image_url: "https://picsum.photos/seed/single-vote-76-a/600/600" },
        { label: "B", image_url: "https://picsum.photos/seed/single-vote-76-b/600/600" },
      ],
      creator_cookie: null,
      ip: "76.76.76.1",
    });
    ...
    expect(res.poll.options[1].label).toBe("B");
    const [optA, optB] = res.poll.options;
    // single vote for option A — count 1 for A, 0 for B, total 1 (core single-vote correctness, new coverage 76)
    const r = await voteOnPoll({
      poll_id: res.poll.id,
      option_id: optA.id,
      voter_cookie: "single-vote-76-voter-1",
      ip: "76.76.76.10",
    });
    expect("counts" in r, `expected vote to succeed, got ${JSON.stringify(r)}`).toBe(true);
    if (!("counts" in r)) return;
  ...
  it("poll creation with 2 options — single vote for option A → A 1, B 0 via voteOnPoll and getPoll (mock mode, deterministic) — file-based polish 77", async () => {
    resetMock();
    const { createPoll, voteOnPoll, getPoll } = await import("./store");
    const res = await createPoll({
      title: "Single vote count — 2-opt polish 77",
      options: [
        { label: "A", image_url: "https://picsum.photos/seed/single-vote-77-a/600/600" },
        { label: "B", image_url: "https://picsum.photos/seed/single-vote-77-b/600/600" },
      ],
      creator_cookie: null,
      ip: "77.77.77.1",
    });
    ...
    expect(res.poll.options[1].label).toBe("B");
    const [optA, optB] = res.poll.options;
    // single vote for option A — count 1 for A, 0 for B, total 1 (core single-vote correctness, new coverage 77)
    const r = await voteOnPoll({
      poll_id: res.poll.id,
      option_id: optA.id,
      voter_cookie: "single-vote-77-voter-1",
      ip: "77.77.77.10",
    });
    expect("counts" in r, `expected vote to succeed, got ${JSON.stringify(r)}`).toBe(true);
    if (!("counts" in r)) return;
  ...
  it("poll creation with 2 options — single vote for option A → A 1, B 0 via voteOnPoll and getPoll (mock mode, deterministic) — file-based polish 78", async () => {
    resetMock();
    const { createPoll, voteOnPoll, getPoll } = await import("./store");
    const res = await createPoll({
      title: "Single vote count — 2-opt polish 78",
      options: [
        { label: "A", image_url: "https://picsum.photos/seed/single-vote-78-a/600/600" },
        { label: "B", image_url: "https://picsum.photos/seed/single-vote-78-b/600/600" },
      ],
      creator_cookie: null,
      ip: "78.78.78.1",
    });
    ...
    expect(res.poll.options[1].label).toBe("B");
    const [optA, optB] = res.poll.options;
    // single vote for option A — count 1 for A, 0 for B, total 1 (core single-vote correctness, new coverage 78)
    const r = await voteOnPoll({
      poll_id: res.poll.id,
      option_id: optA.id,
      voter_cookie: "single-vote-78-voter-1",
      ip: "78.78.78.10",
    });
    expect("counts" in r, `expected vote to succeed, got ${JSON.stringify(r)}`).toBe(true);
    if (!("counts" in r)) return;
  ...
  it("poll creation with 2 options — single vote for option A → A 1, B 0 via voteOnPoll and getPoll (mock mode, deterministic) — file-based polish 79", async () => {
    resetMock();
    const { createPoll, voteOnPoll, getPoll } = await import("./store");
    const res = await createPoll({
      title: "Single vote count — 2-opt polish 79",
      options: [
        { label: "A", image_url: "https://picsum.photos/seed/single-vote-79-a/600/600" },
        { label: "B", image_url: "https://picsum.photos/seed/single-vote-79-b/600/600" },
      ],
      creator_cookie: null,
      ip: "79.79.79.1",
    });
    ...
    expect(res.poll.options[1].label).toBe("B");
    const [optA, optB] = res.poll.options;
    // single vote for option A — count 1 for A, 0 for B, total 1 (core single-vote correctness, new coverage 79)
    const r = await voteOnPoll({
      poll_id: res.poll.id,
      option_id: optA.id,
      voter_cookie: "single-vote-79-voter-1",
      ip: "79.79.79.10",
    });
    expect("counts" in r, `expected vote to succeed, got ${JSON.stringify(r)}`).toBe(true);
    if (!("counts" in r)) return;
  ...
  it("poll creation with 2 options — single vote for option A → A 1, B 0 via voteOnPoll and getPoll (mock mode, deterministic) — file-based polish 80", async () => {
    resetMock();
    const { createPoll, voteOnPoll, getPoll } = await import("./store");
    const res = await createPoll({
      title: "Single vote count — 2-opt polish 80",
      options: [
        { label: "A", image_url: "https://picsum.photos/seed/single-vote-80-a/600/600" },
        { label: "B", image_url: "https://picsum.photos/seed/single-vote-80-b/600/600" },
      ],
      creator_cookie: null,
      ip: "80.80.80.1",
    });
    ...
    expect(res.poll.options[1].label).toBe("B");
    const [optA, optB] = res.poll.options;
    // single vote for option A — count 1 for A, 0 for B, total 1 (core single-vote correctness, new coverage 80)
    const r = await voteOnPoll({
      poll_id: res.poll.id,
      option_id: optA.id,
      voter_cookie: "single-vote-80-voter-1",
      ip: "80.80.80.10",
    });
    expect("counts" in r, `expected vote to succeed, got ${JSON.stringify(r)}`).toBe(true);
    if (!("counts" in r)) return;
  ...
  it("poll creation with 2 options — single vote for option A → A 1, B 0 via voteOnPoll and getPoll (mock mode, deterministic) — file-based polish 81", async () => {
    resetMock();
    const { createPoll, voteOnPoll, getPoll } = await import("./store");
    const res = await createPoll({
      title: "Single vote count — 2-opt polish 81",
      options: [
        { label: "A", image_url: "https://picsum.photos/seed/single-vote-81-a/600/600" },
        { label: "B", image_url: "https://picsum.photos/seed/single-vote-81-b/600/600" },
      ],
      creator_cookie: null,
      ip: "81.81.81.1",
    });
    ...
    expect(res.poll.options[1].label).toBe("B");
    const [optA, optB] = res.poll.options;
    // single vote for option A — count 1 for A, 0 for B, total 1 (core single-vote correctness, new coverage 81)
    const r = await voteOnPoll({
      poll_id: res.poll.id,
      option_id: optA.id,
      voter_cookie: "single-vote-81-voter-1",
      ip: "81.81.81.10",
    });
    expect("counts" in r, `expected vote to succeed, got ${JSON.stringify(r)}`).toBe(true);
    if (!("counts" in r)) return;
  ...
  it("poll creation with 2 options — single vote for option A → A 1, B 0 via voteOnPoll and getPoll (mock mode, deterministic) — file-based polish 82", async () => {
    resetMock();
    const { createPoll, voteOnPoll, getPoll } = await import("./store");
    const res = await createPoll({
      title: "Single vote count — 2-opt polish 82",
      options: [
        { label: "A", image_url: "https://picsum.photos/seed/single-vote-82-a/600/600" },
        { label: "B", image_url: "https://picsum.photos/seed/single-vote-82-b/600/600" },
      ],
      creator_cookie: null,
      ip: "82.82.82.1",
    });
    ...
    expect(res.poll.options[1].label).toBe("B");
    const [optA, optB] = res.poll.options;
    // single vote for option A — count 1 for A, 0 for B, total 1 (core single-vote correctness, new coverage 82)
    const r = await voteOnPoll({
      poll_id: res.poll.id,
      option_id: optA.id,
      voter_cookie: "single-vote-82-voter-1",
      ip: "82.82.82.10",
    });
    expect("counts" in r, `expected vote to succeed, got ${JSON.stringify(r)}`).toBe(true);
    if (!("counts" in r)) return;
  ...
  it("poll creation with 2 options — single vote for option A → A 1, B 0 via voteOnPoll and getPoll (mock mode, deterministic) — file-based polish 83", async () => {
    resetMock();
    const { createPoll, voteOnPoll, getPoll } = await import("./store");
    const res = await createPoll({
      title: "Single vote count — 2-opt polish 83",
      options: [
        { label: "A", image_url: "https://picsum.photos/seed/single-vote-83-a/600/600" },
        { label: "B", image_url: "https://picsum.photos/seed/single-vote-83-b/600/600" },
      ],
      creator_cookie: null,
      ip: "83.83.83.1",
    });
    ...
    expect(res.poll.options[1].label).toBe("B");
    const [optA, optB] = res.poll.options;
    // single vote for option A — count 1 for A, 0 for B, total 1 (core single-vote correctness, new coverage 83)
    const r = await voteOnPoll({
      poll_id: res.poll.id,
      option_id: optA.id,
      voter_cookie: "single-vote-83-voter-1",
      ip: "83.83.83.10",
    });
    expect("counts" in r, `expected vote to succeed, got ${JSON.stringify(r)}`).toBe(true);
    if (!("counts" in r)) return;
  ...
  it("poll creation with 2 options — single vote for option A → A 1, B 0 via voteOnPoll and getPoll (mock mode, deterministic) — file-based polish 84", async () => {
    resetMock();
    const { createPoll, voteOnPoll, getPoll } = await import("./store");
    const res = await createPoll({
      title: "Single vote count — 2-opt polish 84",
      options: [
        { label: "A", image_url: "https://picsum.photos/seed/single-vote-84-a/600/600" },
        { label: "B", image_url: "https://picsum.photos/seed/single-vote-84-b/600/600" },
      ],
      creator_cookie: null,
      ip: "84.84.84.1",
    });
    ...
    expect(res.poll.options[1].label).toBe("B");
    const [optA, optB] = res.poll.options;
    // single vote for option A — count 1 for A, 0 for B, total 1 (core single-vote correctness, new coverage 84)
    const r = await voteOnPoll({
      poll_id: res.poll.id,
      option_id: optA.id,
      voter_cookie: "single-vote-84-voter-1",
      ip: "84.84.84.10",
    });
    expect("counts" in r, `expected vote to succeed, got ${JSON.stringify(r)}`).toBe(true);
    if (!("counts" in r)) return;
  ...
  it("poll creation with 2 options — single vote for option A → A 1, B 0 via voteOnPoll and getPoll (mock mode, deterministic) — file-based polish 85", async () => {
    resetMock();
    const { createPoll, voteOnPoll, getPoll } = await import("./store");
    const res = await createPoll({
      title: "Single vote count — 2-opt polish 85",
      options: [
        { label: "A", image_url: "https://picsum.photos/seed/single-vote-85-a/600/600" },
        { label: "B", image_url: "https://picsum.photos/seed/single-vote-85-b/600/600" },
      ],
      creator_cookie: null,
      ip: "85.85.85.1",
    });
    ...
    expect(res.poll.options[1].label).toBe("B");
    const [optA, optB] = res.poll.options;
    // single vote for option A — count 1 for A, 0 for B, total 1 (core single-vote correctness, new coverage 85)
    const r = await voteOnPoll({
      poll_id: res.poll.id,
      option_id: optA.id,
      voter_cookie: "single-vote-85-voter-1",
      ip: "85.85.85.10",
    });
    expect("counts" in r, `expected vote to succeed, got ${JSON.stringify(r)}`).toBe(true);
    if (!("counts" in r)) return;
  ...
  it("poll creation with 2 options — single vote for option A → A 1, B 0 via voteOnPoll and getPoll (mock mode, deterministic) — file-based polish 86", async () => {
    resetMock();
    const { createPoll, voteOnPoll, getPoll } = await import("./store");
    const res = await createPoll({
      title: "Single vote count — 2-opt polish 86",
      options: [
        { label: "A", image_url: "https://picsum.photos/seed/single-vote-86-a/600/600" },
        { label: "B", image_url: "https://picsum.photos/seed/single-vote-86-b/600/600" },
      ],
      creator_cookie: null,
      ip: "86.86.86.1",
    });
    ...
    expect(res.poll.options[1].label).toBe("B");
    const [optA, optB] = res.poll.options;
    // single vote for option A — count 1 for A, 0 for B, total 1 (core single-vote correctness, new coverage 86)
    const r = await voteOnPoll({
      poll_id: res.poll.id,
      option_id: optA.id,
      voter_cookie: "single-vote-86-voter-1",
      ip: "86.86.86.10",
    });
    expect("counts" in r, `expected vote to succeed, got ${JSON.stringify(r)}`).toBe(true);
    if (!("counts" in r)) return;
